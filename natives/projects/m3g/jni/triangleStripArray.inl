...
(JNIEnv* aEnv, jclass, jlong aM3g, jint first, jintArray aLengthArray)
{
    ...
    if (lengths)
    {
        aEnv->ReleaseIntArrayElements(aLengthArray, (jint *)lengths, JNI_ABORT);
    }
...
(JNIEnv* aEnv, jclass, jlong aHTsa, jintArray aIndices)
{
    ...
    if (aIndices)
    {
        indices = (int *)aEnv->GetIntArrayElements(aIndices, NULL);
        if (indices == NULL)
        {
            M3G_RAISE_EXCEPTION(aEnv, "java/lang/OutOfMemoryError");
            return;
    ...
    if (indices)
    {
        aEnv->ReleaseIntArrayElements(aIndices, (jint *)indices, 0);
    }
}
