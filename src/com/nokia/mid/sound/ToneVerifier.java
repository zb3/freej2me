...
final class ToneVerifier {
	...
	static void fix(byte[] tone) {
		...
		try {
			...
			System.err.println(TAG+": Error parsing tone: " + e.getMessage());
		}
	}
	...
	private void parseTone() {
		...
		if (songType == 1) {
			...
			System.err.println(TAG+": Unsupported ringtone type");
			return;
		}
			...
			for (int k = 0; k < len; k++) {
				...
				switch (id) {
					...
					case 5: //<volume-instruction-id>
						...
						break;
					default:
						System.err.println(TAG+" Unexpected instruction: " + id);
