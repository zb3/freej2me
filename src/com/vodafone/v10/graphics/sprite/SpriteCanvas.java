...
public abstract class SpriteCanvas extends Canvas {
	...
	public void drawFrameBuffer(int tx, int ty) {
		// ?
		platformImage.getCanvas().getGraphics().drawImage(spriteImage.platformImage.getCanvas(), tx, ty, null);
		Mobile.getPlatform().flushGraphics(platformImage, 0, 0, platformImage.width, platformImage.height);
		((PlatformGraphics)graphics).clearRect(0, 0, spriteImage.width, spriteImage.height);
	}
