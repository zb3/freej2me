...
class Loader {
	...
	static Model loadMbacData(byte[] data, int offset, int length) throws IOException {
		...
		if (available > 0) {
			System.err.println(Utils.TAG+": Uninterpreted bytes in MBAC (" + available + ", v=" + version);
		}
	...
	static Action[] loadMtraData(byte[] data, int offset, int length) throws IOException {
		...
		if (transTypeCounts[7] != 0) {
			...
			System.out.println(Utils.TAG+": ActTableData: transTypeCounts[7] = " + transTypeCounts[7]);
		}
		//noinspection unused
		...
		for (int action = 0; action < numActions; action++) {
			...
			final int[] dynamic = new int[count];
			actions[action].dynamic = dynamic;
			for (int j = 0; j < count; j++) {
				int frame = loader.readUShort();
				int pattern = loader.readInt();
				dynamic[frame] = pattern;
			}
		}
		...
		if (available > 0) {
			System.err.println(Utils.TAG+": ActTableData: uninterpreted bytes in MTRA");
		}
	...
	static TextureData loadBmpData(byte[] data, int offset, int length) throws IOException {
		// TODO: we assume this loads to rgba8 aka abgr32 on LE
	...
	private void readPolyC(Model model, int numVertex, int numColor, int numTriangles) throws IOException {
		...
		if (unknownByte != 0) {
			System.out.println(Utils.TAG+": PolyC unknownByte = " + unknownByte);
		}
	...
	private void readPolyV3(Model model, int numVertex, int numTriangles) throws IOException {
		...
		if (unknownByte != 0) {
			System.out.println(Utils.TAG+": PolyT v3: unknownByte = " + unknownByte);
		}
	...
	private int readUBits(int size) throws IOException {
		if (size > 25) {
			System.err.println(Utils.TAG+": readUBits(size=" + size + ')');
