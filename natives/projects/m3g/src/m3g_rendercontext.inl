                                   ...
                                   M3GNativeBitmap bitmapHandle)
{
    ...
    attribs[6].attrib = EGL_RENDERABLE_TYPE;
    attribs[6].value = EGL_OPENGL_ES_BIT;


    if (bitmapHandle) {
        /* This attribute is matched only for pixmap targets */
        // attribs[6].attrib = EGL_MATCH_NATIVE_PIXMAP;
        // attribs[6].value = bitmapHandle; // zb3: gooood luck... this simply doesn't work on 64 bit systems
...
#   if defined(M3G_DEBUG)
    {
        EGLint err = eglGetError();
        if (err != EGL_SUCCESS) {
            M3G_LOG1(M3G_LOG_FATAL_ERRORS, "EGL error %p\n", err);
        }
                                     ...
                                     EGLSurface surface)
{
    ...
    /* No match found, we must create a new context */
    {
        ...
        //if (surfaceTypeBits == SURFACE_EGL) 
        {
            ...
            glrc = eglCreateContext(dpy, m3gQueryEGLConfig(M3G_RGBA8, (M3Gbitmask)(M3G_COLOR_BUFFER_BIT|M3G_DEPTH_BUFFER_BIT), EGL_PBUFFER_BIT, (M3GNativeBitmap)NULL), shareRc, contextAttribs);
            //M3G_ASSERT(glrc);
        }
        /*else {
