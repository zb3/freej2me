...
#   if defined(M3G_DEBUG)
    M3G_LOG4(M3G_LOG_MEMORY_BLOCKS,
             "ObjAlloc %p, %d bytes (%s, line %d)\n",
             handle, bytes, file, line);
#   else
    M3G_LOG2(M3G_LOG_MEMORY_BLOCKS, "ObjAlloc %p, %d bytes\n",
             handle, bytes);
#   endif
...
#   else /* !M3G_DEBUG*/
        M3G_LOG1(M3G_LOG_MEMORY_BLOCKS, "ObjFree %p\n", handle);
#   endif
 ...
 *
 */
static void m3gDeleteGLTextures(Interface *m3g, M3Gsizei n, GLuint *t)
{
...
static void *m3gMapObject(Interface *m3g, M3GMemObject handle)
{
    ...
    if (handle == 0) {
        ...
        M3G_LOG2(M3G_LOG_MEMORY_MAPPING, "MapObj %p -> %p\n",
                 handle, ptr);
